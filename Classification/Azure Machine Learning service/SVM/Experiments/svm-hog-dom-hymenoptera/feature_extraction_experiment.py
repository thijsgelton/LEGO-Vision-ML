...
def train(data_dir, test_dir, classifier_name, number_of_samples, shape, output_directory, beta,
          with_dominant_color_attached, is_local, run):
    start = time.time()
    if test_dir:
...
def main():
    ...
    parser.add_argument('--dominant_color', type=bool, help='with the dominantcolor attached', default=True)
    ...
    if not args.is_local:
        ...
        run.tag('with_dominant_color', str(args.dominant_color))
        run.log('with_dominant_color', str(args.dominant_color))
