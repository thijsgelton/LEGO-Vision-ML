...
from lib.FasterRCNN_eval import compute_test_set_aps, FasterRCNN_Evaluator
from lib.FasterRCNN_train import prepare, train_faster_rcnn, store_eval_model_with_native_udf
from lib.utils.config_helpers import merge_configs
from lib.utils.plot_helpers import plot_test_set_results


def get_configuration():
    from lib.FasterRCNN_config import cfg as detector_cfg
...
def write_results(map, eval_results, cfg, run, is_remote):
    results = dict(
        mAP=map,
    ...
    for metric, value in results.items():
        log_metric(is_remote, metric, value, run)
...
def log_metric(is_remote, metric, value, run=None):
    if is_remote:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fast_mode', default=1, type=int)
    parser.add_argument('--is_remote', default=1, type=int)
    ...
    cfg['CNTK'].FAST_MODE = bool(args.fast_mode)
    ...
    write_results(mAP, eval_results, cfg, run, is_remote)
